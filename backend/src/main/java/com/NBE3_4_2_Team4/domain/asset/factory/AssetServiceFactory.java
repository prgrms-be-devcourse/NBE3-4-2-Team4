...
public class AssetServiceFactory {
    private final Map<AssetType, AssetService> serviceMap;
    ...
    public AssetServiceFactory(List<AssetService> services) {
        ...
        for (AssetService service : services) {
            ...
            serviceMap.put(AssetType.fromValue(key), service);
        }
    }


    public AssetService getService(AssetType type) {
        AssetService assetService = serviceMap.get(type);
        if (assetService == null) throw new IllegalArgumentException("%s인 AssetService 이름이 존재하지 않습니다".formatted(type));
        return assetService;
    }
