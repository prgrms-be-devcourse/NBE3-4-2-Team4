...
@Transactional
public class ApiProductGenFileControllerTest {
    @Autowired
    ...
    void t1() throws Exception {
        ResultActions resultActions = mvc
                .perform(get("/api/products/1/genFiles"))
                .andDo(print());

        resultActions
                .andExpect(handler().handlerType(ApiProductGenFileController.class))
                .andExpect(handler().methodName("items"))
                .andExpect(status().isOk());

        List<ProductGenFile> productGenFiles = productService
                .findById(1L).getGenFiles();

        for (int i = 0; i < productGenFiles.size(); i++) {
            ProductGenFile productGenFile = productGenFiles.get(i);

            resultActions
                    .andExpect(jsonPath("$[%d].id".formatted(i)).value(productGenFile.getId()))
                    .andExpect(jsonPath("$[%d].created_at".formatted(i)).exists())
                    .andExpect(jsonPath("$[%d].parent_id".formatted(i)).value(productGenFile.getParent().getId()))
                    .andExpect(jsonPath("$[%d].type_code".formatted(i)).value(productGenFile.getTypeCode().name()))
                    .andExpect(jsonPath("$[%d].file_ext_type_code".formatted(i)).value(productGenFile.getFileExtTypeCode()))
                    .andExpect(jsonPath("$[%d].file_ext_type2_code".formatted(i)).value(productGenFile.getFileExtType2Code()))
                    .andExpect(jsonPath("$[%d].file_size".formatted(i)).value(productGenFile.getFileSize()))
                    .andExpect(jsonPath("$[%d].file_no".formatted(i)).value(productGenFile.getFileNo()))
                    .andExpect(jsonPath("$[%d].file_ext".formatted(i)).value(productGenFile.getFileExt()))
                    .andExpect(jsonPath("$[%d].file_date_dir".formatted(i)).value(productGenFile.getFileDateDir()))
                    .andExpect(jsonPath("$[%d].original_file_name".formatted(i)).value(productGenFile.getOriginalFileName()))
                    .andExpect(jsonPath("$[%d].download_url".formatted(i)).value(productGenFile.getDownloadUrl()))
                    .andExpect(jsonPath("$[%d].public_url".formatted(i)).value(productGenFile.getPublicUrl()))
                    .andExpect(jsonPath("$[%d].file_name".formatted(i)).value(productGenFile.getFileName()))
                    .andExpect(jsonPath("$[%d].file_type".formatted(i)).value("product"));
        }
    }
    ...
    void t2() throws Exception {
        ...
        ResultActions resultActions = mvc
                .perform(
                        multipart("/api/products/1/genFiles/" + GenFile.TypeCode.attachment)
        ...
        resultActions
                ...
                .andExpect(jsonPath("$.data[0].file_type").value("product"));
    ...
    void t3() throws Exception {
        ...
        ProductGenFile productGenFile = productService
                .findById(1L).getGenFileById(1);

        resultActions
                .andExpect(jsonPath("$.id").value(productGenFile.getId()))
                .andExpect(jsonPath("$.created_at").value(Matchers.startsWith(productGenFile.getCreatedAt().toString().substring(0, 20))))
                .andExpect(jsonPath("$.parent_id").value(productGenFile.getParent().getId()))
                .andExpect(jsonPath("$.type_code").value(productGenFile.getTypeCode().name()))
                .andExpect(jsonPath("$.file_ext_type_code").value(productGenFile.getFileExtTypeCode()))
                .andExpect(jsonPath("$.file_ext_type2_code").value(productGenFile.getFileExtType2Code()))
                .andExpect(jsonPath("$.file_size").value(productGenFile.getFileSize()))
                .andExpect(jsonPath("$.file_no").value(productGenFile.getFileNo()))
                .andExpect(jsonPath("$.file_ext").value(productGenFile.getFileExt()))
                .andExpect(jsonPath("$.file_date_dir").value(productGenFile.getFileDateDir()))
                .andExpect(jsonPath("$.original_file_name").value(productGenFile.getOriginalFileName()))
                .andExpect(jsonPath("$.download_url").value(productGenFile.getDownloadUrl()))
                .andExpect(jsonPath("$.public_url").value(productGenFile.getPublicUrl()))
                .andExpect(jsonPath("$.file_name").value(productGenFile.getFileName()))
                .andExpect(jsonPath("$.file_type").value("product"));
    }
    ...
    void t4() throws Exception {
        ...
        ResultActions resultActions = mvc
                .perform(
                        multipart("/api/products/1/genFiles/" + AnswerGenFile.TypeCode.attachment)
        ...
        resultActions
                ...
                .andExpect(jsonPath("$.data[1].file_type").value("product"));
    ...
    void t5() throws Exception {
        ...
        String originFilePath = productGenFile.getFilePath();
        String copyFilePath = AppConfig.getTempDirPath() + "/copy_" + productGenFile.getFileName();

        Ut.file.copy(originFilePath, copyFilePath);

        ResultActions resultActions = mvc
                .perform(
                        delete("/api/products/1/genFiles/1")
                )
                .andDo(print());
    ...
    void t6() throws Exception {
        ...
        ResultActions resultActions = mvc
                .perform(
                        multipart("/api/products/1/genFiles/1")
        ...
        resultActions
                ...
                .andExpect(jsonPath("$.data.id").value(productGenFile.getId()))
                .andExpect(jsonPath("$.data.created_at").isString())
                .andExpect(jsonPath("$.data.parent_id").value(1))
                .andExpect(jsonPath("$.data.type_code").value(productGenFile.getTypeCode().name()))
                ...
                .andExpect(jsonPath("$.data.file_type").value("product"));
