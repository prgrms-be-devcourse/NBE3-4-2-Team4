...
export default function ClientPage() {
    ...
    const checkTempToken = async () => {
      try {
        const response = await fetch(
          "http://localhost:8080/api/auth/temp-token",
          {
            method: "GET",
            credentials: "include",
          }
  ...
  const checkNicknameAvailability = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/members?nickname=${nickname}`
      );
      if (response.ok) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen space-y-6">
      ...
      <div className="flex flex-col w-full max-w-md space-y-2">
        ...
        {/* 중복 확인 결과 메시지 */}
        {isAvailable === null && (
          <p className="text-gray-600 mt-1">닉네임 중복 확인을 진행하세요.</p>
        )}
        {isAvailable === true && (
          <p className="text-green-600 mt-1">사용 가능한 닉네임입니다!</p>
        )}
        {isAvailable === false && (
          <p className="text-red-600 mt-1">이미 사용 중인 닉네임입니다.</p>
        )}
